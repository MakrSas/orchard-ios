...
pub fn device_scanout_copy(
    ...
    if let Some(ops) = slot.ops {
        ...
        // Headless screenshot: the firmware framebuffer (iBoot logo) only
        // ever flows through this paint path, never through presents.
        if rc == crate::runtime::scanout::ScanoutCopyResult::Painted {
            crate::runtime::drain::maybe_dump_scanout_frame(
        ...
        // Same headless-screenshot hook as the bound arm above: with
        // -display none this is the arm that runs.
        if rc == crate::runtime::scanout::ScanoutCopyResult::Painted {
            crate::runtime::drain::maybe_dump_scanout_frame(
                dst, dst_stride, width, height,
            );
