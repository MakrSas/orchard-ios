...
/// Host-side screenshot for headless runs. When `REIMS_VGPU_PRESENT_DUMP`
/// names a path, writes the presented CPU frame as PPM (BGRA→RGB), throttled
/// to every 600th present (~10 s at 60 Hz). Engine-carried presents have no
/// CPU pixels (empty buffer) and are skipped.
pub(crate) fn maybe_dump_present_frame(frame_bgra: &[u8], w: u32, h: u32) {
    maybe_dump_frame_inner(frame_bgra, w, h, (w as usize).saturating_mul(4));
}

/// Scanout-side screenshot: same file/trigger as the present dump, but for
/// the QEMU console paint path (`device_scanout_copy`), which is what draws
/// the firmware framebuffer (iBoot logo) long before any composited present.
/// `dst` is row-major BGRA with `stride` bytes per row.
pub(crate) fn maybe_dump_scanout_frame(dst: &[u8], stride: u32, w: u32, h: u32) {
    maybe_dump_frame_inner(dst, w, h, stride as usize);
}

fn maybe_dump_frame_inner(frame: &[u8], w: u32, h: u32, stride: usize) {
    use std::sync::atomic::{AtomicU64, Ordering};
    static COUNT: AtomicU64 = AtomicU64::new(0);
    static PATH: std::sync::OnceLock<Option<std::path::PathBuf>> =
        std::sync::OnceLock::new();
    let path = PATH
        .get_or_init(|| std::env::var_os("REIMS_VGPU_PRESENT_DUMP").map(std::path::PathBuf::from));
    let Some(path) = path else {
        return;
    };
    if COUNT.fetch_add(1, Ordering::Relaxed) % 600 != 0 {
        return;
    }
    let w = w as usize;
    let h = h as usize;
    ...
    if ((h as u64).saturating_mul(stride as u64)) as usize > frame.len() {
        return;
    }
    ...
    let mut max_rgb = 0u8;
    for y in 0..h {
        let row = &frame[y * stride..y * stride + row_bytes];
    ...
    // Skip black frames: pre-logo paints and failed clears would overwrite a
    // good earlier shot with 24 MB of nothing (and the 9p write contends).
    if max_rgb == 0 {
        return;
    }
...
fn fill_display_descriptor<H: HostMemory + HostOps>(
    ...
    // there.
    let modes = advertised_display_modes();
    let modes = modes.as_slice();
    let (panel_w, panel_h) = display_panel_mm(modes.first().copied());
    let (width_f32, width_mm) = display_dimension_mm(panel_w);
    let (height_f32, height_mm) = display_dimension_mm(panel_h);
...
/// The timing elements this device offers, in the order it offers them.
///
/// Unset [`crate::config::DISPLAY_MODE`] is the default list, 1920x1080 first.
/// Set, it is the named 16:9 mode first — element 0 is what the guest boots at —
/// followed by the smaller 16:9 modes, largest first, and none bigger. Read once per process and
/// reported once on the failure channel, adopted or refused, because it decides
/// what every later `setupSharedState` publishes.
fn advertised_display_modes() -> Vec<(u16, u16)> {
    static MODES: std::sync::OnceLock<Vec<(u16, u16)>> = std::sync::OnceLock::new();
    MODES.get_or_init(select_display_modes).clone()
}

/// The panel the guest is told it has, in millimetres: the default 48 x 27 cm
/// for a 16:9 native mode, and otherwise the same height with the width that
/// makes the native mode's pixels square, in whole centimetres because that is
/// all the guest's EDID keeps. See `DISPLAY_WIDTH_MM` for why the two must agree.
fn display_panel_mm(native: Option<(u16, u16)>) -> (u16, u16) {
    let Some((w, h)) = native.filter(|(w, h)| *w > 0 && *h > 0) else {
        return (DISPLAY_WIDTH_MM, DISPLAY_HEIGHT_MM);
    };
    let width_cm =
        (u32::from(DISPLAY_HEIGHT_MM) * u32::from(w) + u32::from(h) * 5) / (u32::from(h) * 10);
    let width_mm = (width_cm * 10).clamp(100, 1000) as u16;
    (width_mm, DISPLAY_HEIGHT_MM)
}

/// The bounds a `REIMS_VGPU_DISPLAY_SIZE` mode must sit inside: even sides,
/// no fewer pixels than 640x360 and no more than 1920x1080 holds, and no side
/// past what the scanout path takes.
fn display_size_in_bounds(w: u32, h: u32) -> bool {
    w % 2 == 0
        && h % 2 == 0
        && (320..=2560).contains(&w)
        && (240..=1440).contains(&h)
...
fn select_display_modes() -> Vec<(u16, u16)> {
    match crate::config::dimensions(crate::config::DISPLAY_SIZE) {
        crate::config::Dimensions::Unset => {}
        crate::config::Dimensions::Named(w, h) if display_size_in_bounds(w, h) => {
            crate::observe::fail(format!(
                "display_size_adopted mode={w}x{h} panel_mm={:?}",
                display_panel_mm(Some((w as u16, h as u16)))
            ));
            return vec![(w as u16, h as u16)];
        }
        crate::config::Dimensions::Named(w, h) => {
            crate::observe::fail(format!(
                "display_size_refused value={w}x{h} (out of bounds: even sides, \
                 640x360..1920x1080 pixels; the other modes are offered)"
            ));
        }
        crate::config::Dimensions::Refused(raw) => {
            crate::observe::fail(format!(
                "display_size_refused value={raw:?} (not WIDTHxHEIGHT; the other modes are offered)"
            ));
        }
    }
    let default = vec![
    ...
    let names = DISPLAY_MODE_CHOICES.map(|(name, _, _)| name);
    match crate::config::choice(crate::config::DISPLAY_MODE, &names) {
        crate::config::Choice::Unset => default,
        crate::config::Choice::Refused(raw) => {
            crate::observe::fail(format!(
                "display_mode_refused value={raw:?} allowed={} \
                 (the default modes are offered, {DISPLAY_MODE_EFI_W}x{DISPLAY_MODE_EFI_H} first)",
                names.join(",")
            ));
            default
        }
        crate::config::Choice::Named(name) => {
            // The named mode and the smaller ones, never a larger one. A guest
            // restores the mode it last ran at on a display it has seen before
            // when that mode is on offer: measured on the phone, a macOS guest
            // named 960x540 booted at it and then went back to 1920x1080 at the
            // login window — four times the pixels the ask was meant to save.
            let chosen = DISPLAY_MODE_CHOICES
                .iter()
                .find(|(candidate, _, _)| *candidate == name)
                .map_or(u16::MAX, |(_, width, _)| *width);
            let modes: Vec<(u16, u16)> = DISPLAY_MODE_CHOICES
                .iter()
                .filter(|(_, width, _)| *width <= chosen)
                .map(|(_, width, height)| (*width, *height))
                .collect();
            crate::observe::fail(format!(
                "display_mode_adopted mode={name} modes={}",
                modes
                    .iter()
...
fn present_named_mapping<H: HostMemory + HostOps>(
    ...
    if let Some((w, h, gen)) = paint {
        ...
        if !encoded {
            ...
            // Host-side screenshot: REIMS_VGPU_PRESENT_DUMP=<path> writes the
            // exact CPU frame (PPM) throttled to every 600th present, so a
            // headless run leaves a viewable trace without a window.
            maybe_dump_present_frame(&state.present.frame_bgra, w, h);
        }
...
/// The refresh rate this process advertises and paces VBL at: [`DISPLAY_REFRESH_HZ`]
/// unless [`crate::config::DISPLAY_HZ`] names a slower one. Read once, and
/// reported once on the failure channel when set, adopted or refused.
pub(crate) fn display_refresh_hz() -> u32 {
    static HZ: std::sync::OnceLock<u32> = std::sync::OnceLock::new();
    *HZ.get_or_init(|| {
        const NAMES: [&str; 3] = ["120", "60", "30"];
        match crate::config::choice(crate::config::DISPLAY_HZ, &NAMES) {
            crate::config::Choice::Unset => DISPLAY_REFRESH_HZ,
            crate::config::Choice::Named(name) => {
                let hz = name.parse().unwrap_or(DISPLAY_REFRESH_HZ);
                crate::observe::fail(format!(
                    "display_hz_adopted hz={hz} vbl_interval_us={}",
            ...
            crate::config::Choice::Refused(raw) => {
                crate::observe::fail(format!(
                    "display_hz_refused value={raw:?} allowed={} \
                     (the display runs at {DISPLAY_REFRESH_HZ} Hz)",
                    NAMES.join(",")
                ));
                DISPLAY_REFRESH_HZ
            }
...
/// [`DISPLAY_VBL_MIN_INTERVAL_US`] at the rate [`display_refresh_hz`] chose, so
/// the interrupt the guest is woken by is the rate its timing table names.
pub(crate) fn display_vbl_interval_us() -> u64 {
...
pub(crate) fn claim_display_vbl(last_us: &std::sync::atomic::AtomicU64, now_us: u64) -> bool {
    ...
    let interval = display_vbl_interval_us();
    if gap < interval {
        return false;
    }
    let next = if gap >= 2 * interval {
        now_us
    } else {
        last + interval
    };
