...
/// copy (runtime/writeback_debt.rs) instead of writing it back now.
/// ORCHARD_METAL_EAGER_STORE=1, or the ledger's own REIMS_VGPU_LAZY_WRITEBACK=off,
/// keeps the eager Store.
fn metal_lazy_store() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        !std::env::var("ORCHARD_METAL_EAGER_STORE").is_ok_and(|v| v == "1")
