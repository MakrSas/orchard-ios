...
{
    /*
     * An enum, not a `static const size_t`: the latter makes `hex` a
     * variable-length array in C, which GCC then refuses to initialize. This
     * only ever compiled because upstream builds this file with clang on
     * macOS.
     */
    enum { MAX_LEN = 0x1000 };
