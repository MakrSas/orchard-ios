...
static const ARMCPRegInfo generic_timer_cp_reginfo[] = {
    ...
    /*
     * The counter itself. The AArch64 reads are not ARM_CP_IO in this tree:
     * that flag is for icount's sake — it ends the TB and takes the BQL around
     * the read — and icount is never on here, while the read itself,
     * qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL), is a seqlock and needs no lock.
     * A macOS guest reads CNTVCT_EL0 for every mach_absolute_time(), so the
     * lock and the exit to the main loop were paid all the time.
     */
...
static const ARMCPRegInfo v8_cp_reginfo[] = {
    ...
    { .name = "ELR_EL1", .state = ARM_CP_STATE_AA64,
      .type = ARM_CP_ALIAS | ARM_CP_SUPPRESS_TB_END,
