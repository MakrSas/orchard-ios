...
enum L10n {
    ...
    private static let pairs: [(String, String)] = [
        ...
        ("2.5 ГБ", "2.5 GB"),
        ("Меньше 2 ГБ macOS не загружается: ядро стартует и через несколько секунд перезагружает машину, снова и снова. Потолок процесса на iPhone — около 3 ГиБ, и в него входит всё остальное, что держит приложение: буфер трансляций, графика, сам эмулятор. Если приложение закрывается само через какое-то время после запуска — это iOS отбирает память: уменьшите буфер трансляций. Насколько близко к потолку, пишется в emulator.log строками «Память».",
         "Under 2 GB macOS does not boot: the kernel starts and resets the machine a few seconds later, over and over. The process ceiling on an iPhone is about 3 GiB, and it covers everything else the app holds as well: the translation buffer, the graphics, the emulator itself. If the app closes by itself some time after starting, iOS is taking the memory back: lower the translation buffer. How close it came is in emulator.log, on the \u{201C}Memory\u{201D} lines."),
