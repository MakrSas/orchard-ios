...
## Где мы сейчас — 2026-09-22, 22:45
...
1. **Клавиатура** — сборка 22:43: `inferno_input_key_hid` в библиотеке
   (HID-код → `qemu_input_map_usb_to_linux` → USB-клавиатура vmapple) и
   `GuestKeyboard.swift` в приложении: кнопка слева внизу открывает экранную
   клавиатуру (текст идёт через таблицу US-раскладки), аппаратная клавиатура
   передаёт HID-коды как есть. Проверить вход в систему.
2. Картинка выводится мелко и не по центру — разобраться с раскладкой
   `ScreenView` (скругление от рамки iPhone для macOS уже убрано; альбомная
   ориентация разрешена).
