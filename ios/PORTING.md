...
- **Вывод кадров в приложении** через `CALayer.contents` вместо SwiftUI `Image`
  ...
  клавиша целиком за один захват BQL (`inferno_input_key_tap`).
...
- **Экран**: выбор разрешения (только выбранное и меньшие, иначе macOS
  ...
  исправлено двоение картинки при уменьшении разрешения (`inferno_display_read`).
...
Рычаги без увеличения буфера (сборка 2026-09-22, ещё не замерены):
  ...
  - Сборка 00:09: картинка при 960×540 двоилась и сжималась в верхнюю
    четверть — `inferno_display_read` сообщал о смене размера, только когда
...
**reims-vgpu** — сабмодуль, изменения живут патчами `patches/reims-vgpu/0003…0012`.
В рабочем дереве сабмодуля они уже применены. На свежем клоне:
`git submodule update --init reims-vgpu`, затем `git apply` 0003, 0004, 0005, 0006, 0007, 0008, 0009, 0010, 0011, 0012 по порядку.
